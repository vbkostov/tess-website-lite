...
def make_interactive_paper_plot(
    ...
    fig.write_html(
        plot_path + "papers_over_time.html",
...
def make_static_publication_bar_chart(results, image_path="content/images/statistics/"):
    ...
    start_date = min(publications["month"])  # #Month of first TESS paper
    end_date = max(publications["month"])
    ...
    for ii in range(int(years.count()) - 1):
        ...
        extrapolation = journal_count[-1] * (
            (365 - max(publications["month"]).timetuple().tm_yday) / 365.0
        )
...
def plot_American_authors(
    ...
    fig.write_html(
        plot_path + "US_author_count.html",
        include_plotlyjs="{{ SITEURL }}/theme/js/plotly-latest.min.js",
        default_width="80vw",
        default_height="80vh",
    )
...
def plot_Global_authors(
    ...
    fig.update_coloraxes(colorbar_tickfont={"size": 16})
    fig.write_html(
        plot_path + "Lead_author_affiliation.html",
        include_plotlyjs="{{ SITEURL }}/theme/js/plotly-latest.min.js",
        default_width="80vw",
        default_height="80vh",
    )
